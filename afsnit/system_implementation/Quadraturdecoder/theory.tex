...
\begin{figure}[H]
  ...
  \caption{}
\end{figure}
As mentioned above, a reset is needed for the application. This allows for a new reference point to be set as desired. The state machine does not make use of a clock singal so the reset is desired to be a asyncron reset to avoid adding more complications to the application. The reset should be an asyncron reset, because this does not depend on the clock signal.
