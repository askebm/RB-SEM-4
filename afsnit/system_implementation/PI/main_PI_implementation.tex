...
In order to implement the aforementioned PI-controller in a fashion that complies with API \todo{provided by}from FreeRTOS, it is chosen that a single interface is to be designed. 
\\

\todo{Omskriv sætning, meget mærkelig}This is due to the mattter that only one subprogram has to be written, thus it requieres to be generic, if multiple PI-controllers are to be implemented. 
\\
...
In this the desired calculations will be done. This is done to meet the first criteria and is achieved in \todo{ref til intet}\ref{list:funtion_body}.

\lstinputlisting[language=C, firstline=3, lastline=16,label={list:funtion_body} ]{\main/afsnit/system_implementation/PI/code_snippets.h}



\todo{Svær sætning, og det er en task creation call ikke en tas ki sig selv som får argumentet}tsk\_PI() will be the function which the task created by FreeRTOS will have as an argument.
...
The struct members can be seen in \ref{list:struct_declar}.\todo{ref til intet} 
\lstinputlisting[language=C, firstline=18, lastline=31,label={list:struct_declar}]{\main/afsnit/system_implementation/PI/code_snippets.h}
\todo{Har fjernet en newpage, det fucker med flow i hele rapporten}
%\newpage
...
To get the PI equation into the time-discrete z-domain, it is chosen to use \todo{Skal "the" være her?}the Tustin's Method.
...
Equation \todo{Ref}ref??  now becomes
$$
...
\lstinputlisting[language=C, firstline=35, lastline=64,label={list:PI_controller}]{\main/afsnit/system_implementation/PI/code_snippets.h}
