...
The overarching aim of the project is to create a platform for a stage spotlight with the behaviour expected of such a system in regards to its movement characteristics.
...
\todo{A communication's protocol should be designed and implemented on the FPGA with SPI as the communication method. } Make the FPGA able to communicate by using SPI and implement a protocol.
...
These specifications consists of rise time, which is the time it takes to reach a \todo{which is the time it takes to reach 90\% of the steady-state response.} given set point with a threshold.
