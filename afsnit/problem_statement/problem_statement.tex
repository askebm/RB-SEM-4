...
These specifications consists of rise time,\todo{which is the time it takes the system to surpass the interval from 10\% to 90\% } which is the time it takes to reach a given set point with a threshold.
...
\todo{ved godt det er sent, men er det ikke meget høje krav? plus hvis afstanden er stor, så er kravet for risetime og settling time jo mindre? Plus det medvirker jo også bare til et endu større behov for d leddet, for at minimere overshoot?}
...
\todo{Risetime 90\% ? det ser mærkeligt ud.}
\begin{table}[h]
	\centering
	\begin{tabular}{ll}
		Overshoot & 0\\
