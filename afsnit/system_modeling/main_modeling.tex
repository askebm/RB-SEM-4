...
\label{ch:Top_frame}
To determine the top frame's moment of inertia, $J_{t}$, its seen as an isolated system with only the frames contributing to $J_{t}$, thus the weight of motors and wires on the frame are considered negligible. The system then considered is seen on figure \ref{[REF TO TopFrame.svg]}. Here the axis of rotation can be seen and for this calculation will be our reference.\\
\textbf{Insert TopFrame.svg - iterative naming of components}\\
...
$J_{b,cont}$ can now de determined as shown in eq. (\ref{eq:J_b,cont}).
...
Thus the constant base frames moment of inertia is determined to be $J_{b,const} = 0.0304 \si{kg\cdot m^2}$
\\
