...
In this project a pan-tilt system has been given to control. \todo{A pan-tilt system is provided for the project. The goal is to control this}The system can be seen on figure \ref{fig:ImageOfPanTiltSystem}.
\todo{Undgå ordet "goal" brug "aim" eller "Purpose" - Mindre talesprog}
\begin{figure}[h]
  \centering
...
By further inspection \todo{inspecting eller inspection of} a non-linear contribution on the top frame's moment of inertia is caused by the added aluminium corners as seen on figure (\ref{fig:real_image_of_pan-tilt_system}).\\
...
The added term in eq. (\ref{eq:new_mech_equation_with_corners}) contains the gravitational acceleration on earths surface, $g = 9.82 \si{\frac{m}{s^2}}$, $\theta_t$ which is the angle of the top frame with the vertical axis of the system being the reference and $m_{t,cor}$ \todo{The $m_{t,cor}$is hard to read and should be changed} being the mass of 1 of the 2 the aluminium corners on the top frame.\\
...
To apply the models found in section \ref{ch:General_model} on page \pageref{ch:General_model} the various constants in this model \todo{we are referencing the same model(S) twice, can it be chande?} must be determined. Since the pan-tilt system contains two motors, one for the top frame, and one for the bottom frame, the constants concerning these will from this point on be denoted with the following subscripts, in the same order as mentioned above: $K_{t,a}$ and $K_{b,a}$. \todo{way too long a sentece with way too many interpolated sentences}\\
...
To determine the top frame's moment of inertia, $J_{t}$, it is considered to be an isolated system with only the frames \todo{do you mean both frames or do you mean frame's} contributing to $J_{t}$ as the weight of motors and wires on the frame are considered negligible. The system then considered \todo{The new model of the system is shown in figure \ref{fig:TopFrame}} is seen on figure \ref{fig:TopFrame}. The axis of rotation, shown in the figure \ref{fig:TopFrame} will be the reference for the following calculation.\\
...
In this expression $I_{parallel axis}$ \todo{maybe a bit too long variable name} is the resulting moment of inertia, $I_{cm}$ is the center of mass inertia, $m$ is the mass of the object and $u$ is the distance between the axis of rotation and the axis going through the center of mass parallel to the axis of rotation.

It is here seen that $d$ has two corners \todo{where is that seen? }, $m_{cor}$, which here \todo{which in this particular example? something else than here will be seen as} will be seen as added mass to $d$. As a final concideration the length of $b$ and $d$ are two times the bar width, which is found to be $L_{bw} = 0.04m$, shorter than $a$ and $c$. With these criteria, the moment of inertia can be determined as seen in eq. (\ref{eq:Top_frame_total_inertia}).
...
First the constant part of the moment of inertia is determined which, with the same procedure, results in eq. \todo{same precedure? i know what you mean, but not sure if it would work} (\ref{eq:top_frame_constant}).
...
Since the various parameters in the found models \todo{lidt for fordansket sætning  - f.eks. med "now are determined", "in the found model"}now are determined with $K_{b,torque/elec}$, $K_{t,torque/elec}$, $b_b$ and $b_t$ being the exceptions. The two models can thus be written as seen \odp{maybe say: can now be put on state space form or similar?} in eq. (\ref{eq:Theoretical_models_top}) for the top and (\ref{eq:Theoretical_models_bottom}) for the bottom. Since we \todo{WE hehe} know that the gearing between the motors and frames are $3:1$, each moment of inertia gets divided by 3.
\begin{equation}
