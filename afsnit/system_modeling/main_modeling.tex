...
The first step in control engineering is to create a model of the physical system which requires control, in this context the pan-tilt systems two axis. The purpose with this section is therefore to formulate an approximated model of the system as a true representation cannot be achieved due to the complexity and interplay of the real system.
\\


By analysis of the forces acting on the motors when voltage is applied, the following differential equations are found.\\
...
The state space models for the two motors can be written as shown in eq. (\ref{eq:ss_topframe_Genereal_model}) and (\ref{eq:ss_bottomframe_Genereal_model}).
\todo{which is which?}
...
\end{split}
\end{equation}
\todo{Do we need to explain the moment of inertia formula?}
...
The varying moment of inertia, $J_{b,var}(\theta_t)$, is dependent on the angular diaplacement of the top frame, from the vertical axis parallel to the bottom frame. This can be seen on figure (\ref{fig:TrekantDiagram}).
...
Since the different parameters in the models now are determined with $K_{b,torque/elec}$, $K_{t,torque/elec}$, $b_b$ and $b_t$ being the exceptions. These models can thus be written as seen in eq. (\ref{eq:Theoretical_models_top}) and (\ref{eq:Theoretical_models_bottom}).
\todo{add reference for top and bottom frame}
\begin{equation}
