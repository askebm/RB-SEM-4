...
While the top frame of the pan-tilt system is nonlinear, the bottom frame is not. Therefore it is not necessary to linearize it. But as top frame can move, the moment of inertia of the bottom and top frame is a function of the top frames angle in relation to the bottom frame, as described in section \ref{sec:moment_botom}. \todo{As the top frame can move, it means it influences the moment of inertia of the bottom frame as a function of the top frames angle in relation to the bottom frame, as described in section \ref{sec:moment_botom}}
...
In figure \ref{fig:rlocus_pure_bot_90} a root locus of the openloop system for $90^o$ is seen. Figure \ref{fig:rlocus_int_bot_90} shows the root locus of the system with an I-controller with gain $T_i = \frac{1}{0.035}$.
...
A gain of 9 will draw one of the poles from $0$ to the zero, and place a pole in -5, which will dominate the system. This is needed for the system to comply with the requirements as explained in section \ref{sec:top_frame_tuning_90}. \todo{vi skriver PRÆCIS det samme for $0^o$ - ordret. Det er ok det er det samme, men så skal vi skrive noget andet}
...
\begin{table}[h]
	 ...
	 \begin{tabular}{ll}
		 Overshoot & 0.6315\%\\
		 Risetime  & 0.4099s\\
		 Settling time & 0.7071s
	 \end{tabular}
	 \caption{Performance characteristics of PI-controlled system in figure \ref{fig:step_0_first_bot_90}.}
	 \label{tab:performance_0_7}
\end{table}



Figure \ref{fig:step_0_first_bot_90} shows the step response of that PI-controller.
The response is not as expected, as the system is not a first order system. Therefore the P gain is tuned to better meet requirements. $K_p = 3.7$ returns the step response seen in figure \ref{fig:step_0_second_bot_90}.
...
\begin{table}[h]
	 ...
	 \begin{tabular}{ll}
		 Overshoot & 1.6636\%\\
		 Risetime  & 1.0755s\\
		 Settling time & 1.7388s
	 \end{tabular}
	 \caption{Performance characteristics of PI-controlled system in figure \ref{fig:step_0_second_bot_90}.}
	 \label{tab:performance_0_8}
\end{table}



In figure \ref{fig:comp_bot} a graph of both step responses are shown. It shows that there is no difference regardless of the top frame being at an angle of $0^o$ or at $90^o$, when the two plants with the same cotroller is tested. Based on this, it is concluded that the changing moment of inertia does not have an effect on the response of the system.
