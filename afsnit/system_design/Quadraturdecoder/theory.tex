...
The decoder works by first initializing and go to a state depending on the levels of signal A and B. The decoder will then use the debounced signals to determine the next state.
...
The advantages of a synchronous reset is that it gives a synchronous circuit and provides a natural filter for most glitches.\footnote{If a glitch/noise happens on an active clock edge the fake reset will go through}
