...
\todo{I den første sætning skal der måske lige byttes rundt på ref og tekst: in the subsection ref, regarding the Quadrature Decoder,....}
...
This is a very inefficient way of doing so, as it is not possible to home the system without turning it on and off and the dc-motors has to be turned manually. \todo{turning the system and dc-motors on and off manually, ville måske være en bedre formuleringsmåde.}
...
\end{figure}
\todo{end stop skal nok være med bindestreg, ellers giver det ikke så meget mening.}
\paragraph{Method 1}%
\label{par:method_1}
One way of homing the system is to give the top frame a low PWM signal, so \todo{so = such?} that it is rotating slowly.
When the top frame triggers the hall sensor it resets the position counter for the top frame.
Then the reference for the controller is set to zero, and the frame moves to $0^\circ$.\todo{Vi kan bruge celsius i stedet, men fan af løsningen xD}
\\
For the bottom frame an extra complexity is added in the form of the end stop\todo{end stop = end-stop?}. It is necessary to be able to determine if the frame is hitting the end stop when it is trying to move to the desired position.
If it is hitting the end stop, it needs to switch direction of the motor for the frame to be able to return to home position.\todo{lidt knørklet formulering}
...
The system is also able to detect if the bottom frame is hitting the end stop. If this happens while homing the direction of the bottom frame is switched. \todo{er ikke sikker på hvad der menes i denne sætning}
